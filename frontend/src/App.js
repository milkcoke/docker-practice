...
const containerDomain = "3.35.12.135"
...
function App() {
  ...
  useEffect(function () {
    async function fetchData() {
      ...
      try {
        ...
        const response = await fetch(`http://${containerDomain}:${BACKEND_PORT}/goals`);
  ...
  async function deleteGoalHandler(goalId) {
    ...
    try {
      const response = await fetch(`http://${containerDomain}:${BACKEND_PORT}/goals` + goalId, {
        method: 'DELETE',
      });
